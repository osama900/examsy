...
function renderDragDrop(question, qIndex) {
  ...
  // Note: This logic assumes words are unique. If the correct answer has duplicates, 
  // we might need a more complex state, but for now this matches the user request.
  ...
  const checkAndAwardCoins = async (currentDocId) => {
    ...
    try {
      ...
      examsSnapshot.forEach(doc => {
        if (doc.id === currentDocId) return; // Skip the attempt we just saved
